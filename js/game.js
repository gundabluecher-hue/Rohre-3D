...
function rebuildPortals() {
  ...
  // VERSETZT: Portale nicht mehr direkt gegenÃ¼ber!
...
function buildLabyrinth() {
  ...
  for (let r = 0; r <= rows; r++) {
    for (let c = 0; c <= cols; c++) {
      if (Math.random() > 0.35) continue; // LÃ¼cken lassen
...
function initPlayers() {
  ...
  const extraBots = parseInt(document.getElementById("numExtraBots")?.value) || 0;
  ...
  // Player 2 (Red) - Only if 2-Player-Mode is explicitly selected or requested via old boolean logic?
  // User wants "bots extra". So if Mode=1, we have P1 + ExtraBots.
  // If Mode=2, we have P1 + P2 + ExtraBots.
  // Exception: If Mode=1 BUT "Player 2 is Bot" is checked, we spawn P2 as bot? 
  // Probably yes, for backward compat.
  const chkBot2 = document.getElementById("chkBot2") || document.getElementById("chkBot");
  const p2BotRequested = chkBot2 && chkBot2.checked;

  if (count > 1 || p2BotRequested) {
  ...
  for (let i = 0; i < extraBots; i++) {
    const id = 100 + i;
    // Generate distinct color (Golden/Purple/Green variants)
    const hue = (i * 137.5 + 40) % 360;
    const color = new THREE.Color(`hsl(${hue}, 80%, 60%)`);

    // Create
    const bot = makePlayer(id, color);

    // Initial Position (Randomized spawn around center)
    const offset = new THREE.Vector3(
      (Math.random() - 0.5) * 400,
      (Math.random()) * 200,
      (Math.random() - 0.5) * 400
    );
    // Overwrite spawn pos in reset later, but set here too
    // Note: resetPlayer uses state.editor.spawnPos... which is fixed.
    // We might need to disperse them in resetPlayer or here.
    // See resetPlayer logic.

    players.push(bot);
    scene.add(bot.trailGroup);
  }
}
...
function handleBounds(p) {
  ...
  // Bot ist unsterblich - aber soll nicht immer wrappen, sondern abprallen
  const chkBot = document.getElementById("chkBot");
  const isBot = chkBot && chkBot.checked && p.id === 2;

  // Wenn Wrap AN ist: Wrappen (Teleport)
  if (chkWrap.checked) {
  ...
  // Wenn Bot: Clampen (an Wand stoppen), damit er nicht stirbt
  if (isBot) {
    p.pos.x = clamp(p.pos.x, -halfW2, halfW2);
    p.pos.z = clamp(p.pos.z, -halfD2, halfD2);
    p.pos.y = clamp(p.pos.y, CONFIG.WALL_MARGIN, halfH2);
    return true;
  } else {
...
function updateNPC(dt) {
  ...
  // Bounce Logic (3D Box Wrap/Bounce)
  const margin = 25; // Small margin to prevent clipping
  let bounced = false;
  const normal = new THREE.Vector3();

  // X Walls
  if (pos.x < -halfW + margin) {
    pos.x = -halfW + margin;
    normal.set(1, 0, 0);
    bounced = true;
  } else if (pos.x > halfW - margin) {
    pos.x = halfW - margin;
    normal.set(-1, 0, 0);
    bounced = true;
  }
  ...
  // Z Walls
  if (pos.z < -halfD + margin) {
    pos.z = -halfD + margin;
    normal.set(0, 0, 1);
    bounced = true;
  } else if (pos.z > halfD - margin) {
    pos.z = halfD - margin;
    normal.set(0, 0, -1);
    bounced = true;
  }
  ...
  if (pos.y < margin) {
    ...
    normal.set(0, 1, 0);
    bounced = true;
  } else if (pos.y > CONFIG.ARENA_H - margin) {
    pos.y = CONFIG.ARENA_H - margin;
    normal.set(0, -1, 0);
    bounced = true;
  }
  ...
  if (bounced) {
    ...
    // 2. Anti-Perpendicular Logic (Avoid 90 degree bounces)
    // Check if new velocity is too parallel to normal (meaning it was flying straight at it)
    // Actually, after reflection, if it WAS straight, it IS straight back.
    // We want to ensure it glances off.

    // Dot product of Velocity(normalized) and Normal. 
    // If |dot| approx 1 -> Flying straight along normal.
    const vNorm = vel.clone().normalize();
    const dot = vNorm.dot(normal); // Should be > 0 (pointing away) after reflect

    // Ideally we want a shallow impact. If dot is too high (close to 1), it means steep angle.
    if (dot > 0.85) {
      // Perturb velocity to make it less steep
      // Add a random vector perpendicular to normal
      const randVec = new THREE.Vector3(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5);
      // Project onto tangent plane
      const tangent = randVec.sub(normal.clone().multiplyScalar(randVec.dot(normal))).normalize();

      // Add significant tangent component to velocity
      vel.addScaledVector(tangent, vel.length() * 0.6);
      vel.normalize().multiplyScalar(280); // Reset speed
    }
  }

  // Obstacle Bounce (Simple Sphere)
  obstacles.forEach(ob => {
    const dist = pos.distanceTo(ob.mesh.position);
    const radSum = ob.radius + 20;
    if (dist < radSum) {
      const n = pos.clone().sub(ob.mesh.position).normalize();
      vel.reflect(n);
      pos.copy(ob.mesh.position).addScaledVector(n, radSum + 2);
    }
  });
...
function resetPlayer(p) {
  ...
  if (p.id === 2 && players.length > 1) {
    ...
    // Extra Bots: Random Circle Distribution
    const idx = p.id - 100;
    const angle = idx * 1.5; // rad
    const r = 200 + (idx * 50);
    p.pos.x += Math.cos(angle) * r;
    p.pos.z += Math.sin(angle) * r;
    p.pos.y += (idx % 3) * 60; // Spread Height
  }
  p.q.identity();
...
function step(dt) {
  ...
  players.forEach((p, idx) => {
    // Checkbox mapping: P1 -> chkBot1, P2 -> chkBot2 (formerly chkBot)
    // Note: older HTML might only have chkBot (P2). We need to support new chkBot1.
    const isBot = (p.id === 1)
      ? (document.getElementById("chkBot1")?.checked)
      : (document.getElementById("chkBot2")?.checked || document.getElementById("chkBot")?.checked);

    if (isBot) {
      // Find enemy (first one that is not me)
      const enemy = players.find(x => x.id !== p.id);
      updateBot(p, enemy, dt);
    }
  });
...
function animate() {
  ...
  // Split-Screen logic:
  // Only use split screen if we have a Player 2 AND Player 2 is NOT a bot (or user wants split for bot?)
  // User complained about split screen with bots. Usually bots don't need a view.
  const p2 = players.find(p => p.id === 2);
  const chkBot2 = document.getElementById("chkBot2") || document.getElementById("chkBot");
  const p2IsBot = chkBot2 && chkBot2.checked;
  const isTwoPlayerMode = document.getElementById("selPlayers")?.value === "2";

  // Rule: Split screen only if 2-Player Mode is active OR there is a Human P2.
  // If P2 is a bot, we default to Single View (spectating P1 fighting P2).
  const useSplitScreen = p2 && !p2IsBot && isTwoPlayerMode;
...
function boot() {
  ...
  if (!window.WebGLRenderingContext) {
    showOverlay("Fehler", "Dein Browser unterstÃ¼tzt kein WebGL. Das Spiel kann nicht gestartet werden.", "error");
    return;
  }
  ...
  showOverlay(
    ...
    `âœ… Dropdown fÃ¼r zwei Maps: Standard Arena & komplexes Labyrinth.<br>
         âœ… Portale in den WÃ¤nden verbinden Seiten & Front/Back.<br>
         âœ… Auto-Roll-Option mit Slider fÃ¼r StÃ¤rke.<br>
         âœ… Boost mit Ladebalken (Shift).<br>
         âœ… 1st/3rd-Person Kamera (C).`,
    "start"
  );
...
if (btnExitEditor) {
  btnExitEditor.addEventListener("click", () => {
    setPhase("idle");
    showOverlay("Karte gespeichert", "Deine Ã„nderungen wurden Ã¼bernommen.", "start");
  });
}
